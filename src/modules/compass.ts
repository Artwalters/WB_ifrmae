...
export function initCompass(map: Map): void {
  ...
  // Ticks at every 45° (N, NE, E, SE, S, SW, W, NW)
  let ticks = ''
  for (let deg = 0; deg < 360; deg += 45) {
    const isCardinal = deg % 90 === 0
    const rad = (deg * Math.PI) / 180
    const outerR = 23
    const len = isCardinal ? 3.5 : 2.5
  ...
  // Update rotation and sync currentBearing on map rotate
  const updateBearing = () => {
  ...
  // Click to rotate 45° each time (full 360°)
  btn.addEventListener('click', () => {
    currentBearing = (currentBearing + STEP_SIZE) % 360
    map.easeTo({ bearing: currentBearing, pitch: 40, duration: 600 })
  })
}
